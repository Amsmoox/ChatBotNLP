#Hello, GUI Code of a Chatbot , we are using Tkinter
# Libaries 
from tkinter import *
import time
...
# Menu bar
    ...
    # Options
        ...
        help_option.add_command(label="About MayoubBot", command=self.msg)
    ...
    def last_sent_label(self, date):
        ...
        try:
            self.sent_label.destroy() # Destroy any existing label
        except AttributeError:
            pass

        # Create and display a new label showing the last sent date
        self.sent_label = Label(self.entry_frame, font="Verdana 7", text=date, bg=self.tl_bg2, fg=self.tl_fg)
        self.sent_label.pack(side=LEFT, fill=X, padx=3)

    # Function to clear the chat messages and display a 'No messages sent' label
    def clear_chat(self):
        self.text_box.config(state=NORMAL)
        self.last_sent_label(date="No messages sent.") # Update the last sent label with a message
        self.text_box.delete(1.0, END) # Delete all chat messages
        self.text_box.delete(1.0, END) # Delete all chat messages (again)
        self.text_box.config(state=DISABLED)
    
    # Function to exit the chatbot application
    def chatexit(self):
        exit()

    # Function to display an information message about the chatbot version , you find it in menu
    def msg(self):
        tkinter.messagebox.showinfo("MayoubBot v1.0")

    # Function to display information about the developer in menu
    def about(self):
        tkinter.messagebox.showinfo("MayoubBot Developer","Mharrech Ayoub")
    
    # Function to handle sending and displaying user messages and bot responses
    def send_message_insert(self, message):
        user_input = self.entry_field.get()
        pr1 = "Human : " + user_input + "\n"
        self.text_box.configure(state=NORMAL)
        self.text_box.insert(END, pr1) # Insert the user's message in the text box
        self.text_box.configure(state=DISABLED)
        self.text_box.see(END) # Scroll to the end of the chat

        # Call the chat function to get the bot's response
        ob=chat(user_input)
        pr="MAyoubBot : " + ob + "\n"
        self.text_box.configure(state=NORMAL)
        ...
        time.sleep(0)
        # Start a new thread to play the bot's response
...
root.title("MAyoubBot")
#we can add an icon too >> root.iconbitmap('iconname.ico')
root.mainloop()